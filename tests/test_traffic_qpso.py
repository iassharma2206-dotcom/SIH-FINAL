...
# ── 6. Traffic Audit Logging Tests ──────────────────────────────────────────

def test_traffic_audit_log_and_summary():
    """Verify truthful audit attribution for live_api, cache, and fallback calls."""
    clear_audit_log()
    assert len(get_audit_log()) == 0
    empty_summary = get_audit_summary()
    assert empty_summary['total_calls'] == 0
    assert empty_summary['live_data_pct'] == 0.0

    # 1. Test fallback log entry (no key or invalid coords)
    with patch("traffic_provider._get_tomtom_key", return_value=None):
        res_fb = get_traffic_flow(26.9124, 75.7873)
        assert res_fb['ratio'] == 1.0

    log = get_audit_log()
    assert len(log) == 1
    assert log[0]['source'] == 'fallback'
    assert log[0]['lat'] == 26.9124
    assert log[0]['lon'] == 75.7873
    assert log[0]['ratio'] == 1.0

    # 2. Test live_api log entry followed by cache hit
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_resp.json.return_value = {
        "flowSegmentData": {"currentSpeed": 35, "freeFlowSpeed": 50, "confidence": 0.85}
    }
    with patch("requests.get", return_value=mock_resp), \
         patch("traffic_provider._get_tomtom_key", return_value="dummy_key"), \
         patch.dict("traffic_provider._TRAFFIC_CACHE", {}, clear=True):
        res_live = get_traffic_flow(26.9855, 75.8513)
        assert res_live['ratio'] == 0.7

        log = get_audit_log()
        assert len(log) == 2
        assert log[1]['source'] == 'live_api'
        assert log[1]['ratio'] == 0.7
        assert log[1]['confidence'] == 0.85

        # 3. Test cache hit log entry
        res_cache = get_traffic_flow(26.9855, 75.8513)
        assert res_cache['ratio'] == 0.7

        log = get_audit_log()
        assert len(log) == 3
        assert log[2]['source'] == 'cache'
    ...
    assert np.isclose(summary['live_data_pct'], 100.0 / 3.0)
    assert summary['first_call'] is not None
    assert summary['last_call'] is not None

    # 5. Verify JSON export
    with tempfile.TemporaryDirectory() as tmpdir:
        export_file = os.path.join(tmpdir, "test_audit.json")
        out_path = export_audit_log(export_file)
        assert os.path.exists(out_path)
        with open(out_path, "r", encoding="utf-8") as f:
            import json
            saved_data = json.load(f)
            assert len(saved_data) == 3
            assert saved_data[0]['source'] == 'fallback'
            assert saved_data[1]['source'] == 'live_api'
            assert saved_data[2]['source'] == 'cache'
