...
def run_benchmark_comparison(run_id: str):
    ...
    Runs all 4 algorithms on identical dataset and matrices. Fast-cached for instant responses.
    """
    if run_id in BENCHMARK_CACHE:
        return BENCHMARK_CACHE[run_id]
    ...
    nodes = [start_node] + stops_data[:8] # Limit to 8 stops for ultra-fast HK exact solver execution
    dist_mat, time_mat = build_distance_matrix(nodes)
